...
fn run(args: &Args) -> Result<(), Box<Error>> {
    ...
    println!("loading geojson from repository located around {}", wof_path);

    let repo = Repository::open(wof_path)?;
    let index = repo.index()?;

    let paths: Vec<_> = index.iter()
        .map(|index_entry| { 
            String::from_utf8(index_entry.path).unwrap()
         })
    ...
    paths.par_iter().for_each (|path| {
        ...
        println!("lookin at: {} ..  file? {}", file_path.display(), file_path.is_file());
